...
When a Proposal is sent to a domain Review endpoint, the Host forwards only the stable attachment references explicitly selected for that Proposal, together with its source references. The Host rejects references that were not uploaded in the current interaction. Domains can therefore retain their own evidence links without receiving file bytes or inheriting unrelated attachments from another domain fact.
